namespace perl {
	namespace typecast {
		...
		template<typename T> struct typemap<T, typename boost::enable_if<typename boost::is_convertible<const Scalar::Value, T>::type>::type> {
			...
			static const Scalar::Temp cast_from(interpreter* pre_interp, const T& t) {
		...
		template<typename T> struct typemap< std::vector<T> > {
			...
			static const Array::Temp cast_from(interpreter* pre_interp, const std::vector<T>& array) {
				Array::Temp ret = Interpreter(pre_interp).list();
